...
import { CheckCircle, ArrowRight, Sparkles, Camera } from "lucide-react";
    ...
    <main>
      ...
      <section id="occasions" className="py-20 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            {PERSONAL_OCCASIONS.filter((o) => o.popular).map((occasion) => {
              ...
              return (
                <div
                  key={occasion.title}
                  className="border border-card-border overflow-hidden hover:border-red/30 transition-all duration-300 group"
                >
                  ...
                  <div className="p-6">
                    {"badge" in occasion && occasion.badge && (
                      <span className="inline-flex text-[10px] font-bold uppercase tracking-wider text-red bg-red/10 px-2 py-0.5 mb-3">
                        {occasion.badge}
                      </span>
                    )}
                    <h3 className="text-lg font-semibold text-heading mb-2">{occasion.title}</h3>
                    <p className="text-sm text-muted leading-relaxed">{occasion.description}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="border-t border-card-border bg-background-alt p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-4">
              {PERSONAL_OCCASIONS.filter((o) => !o.popular).map((occasion) => (
                <div key={occasion.title}>
                  <p className="text-sm font-semibold text-heading leading-tight">{occasion.title}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-0">
            {PERSONAL_BANNER_OPTIONS.map((option) => (
              <div key={option.title} className="border-t border-card-border py-6">
                <h3 className="text-base font-semibold text-heading mb-2">{option.title}</h3>
                <p className="text-sm text-muted leading-relaxed mb-3">{option.description}</p>
                <p className="text-xs text-red font-medium mb-4">Best for: {option.bestFor}</p>
                <a
            ...
            <div className="flex justify-center gap-16 mb-10">
              {SMOKE_COLORS.map((smoke) => (
                <div key={smoke.color} className="text-center">
                  <div
                    className="w-20 h-20 rounded-full mx-auto mb-3 border-2 border-card-border"
                    style={{ backgroundColor: smoke.hex }}
                  />
                  <span className="text-sm text-muted font-medium">{smoke.color}</span>
                </div>
              ))}
            </div>
      ...
      <section className="py-20 bg-background-alt">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto border-t border-card-border pt-8">
